package ru.job4j.function;

import org.junit.jupiter.api.DisplayName;
import org.junit.jupiter.api.Test;

import java.util.List;
import java.util.function.Predicate;

import static org.hamcrest.MatcherAssert.assertThat;
import static org.hamcrest.Matchers.*;

class SearchFolderTest {

    List<Folder> list = List.of(
            new Folder("fix", 110),
            new Folder("bug", 75),
            new Folder("bug", 90)
    );

    @Test
    @DisplayName("Test filter when filter size")
    void whenFilterSize() {
        List<Folder> expected = List.of(new Folder("fix", 110));
        Predicate<Folder> pred = f -> f.getSize() > 100;
        List<Folder> out = SearchFolder.filter(list, pred);
        assertThat(out, is(expected));
    }

    @Test
    @DisplayName("Test filter when filter name")
    void whenFilterName() {
        List<Folder> expected = List.of(
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        Predicate<Folder> pred = f -> f.getName().contains("bug");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }

    @Test
    @DisplayName("Test filter when filter name")
    void whenFilterNameButNoMatch() {
        List<Folder> expected = List.of(
        );
        Predicate<Folder> pred = f -> f.getName().contains("lkk");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }
}